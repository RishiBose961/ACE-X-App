...
const loadUserFromStorage = async (): Promise<any | null> => {
  try {
    const userInfo = await AsyncStorage.getItem("activeAccount");
...
const authSlice = createSlice({
  ...
  initialState,
  reducers: {
    loginUserAction: (state, action: PayloadAction<any>) => {
      state.user = action.payload; // Replace `any` with your user type
      state.isAuthenticated = true;
      state.isLoading = false;
      AsyncStorage.setItem("activeAccount", JSON.stringify(action.payload));
    },
    ...
    setUserAction: (state, action: PayloadAction<any>) => {6
      state.user = action.payload; // Replace `any` with your user type
      state.isAuthenticated = true;
      state.isLoading = false;
    },
